...
func (c *DNSProvider) Present(domain, token, keyAuth string) error {
	...
	return c.updateRecords(rec, domainZone)
}

func (c *DNSProvider) updateRecords(records []DNSRecord, domainZone string) error {
	...
	resp, err = c.makeRequest("PATCH", fmt.Sprintf("/v1/domains/%s/records", domainZone), bytes.NewReader(body))
	if err != nil {
		return err
	}
